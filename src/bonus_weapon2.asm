...
timer.weapon2 resd 2

section .text

;init(dw row.offset, dw col.offset)
; Initialize a weapon2
global bonus_weapon2.init
bonus_weapon2.init:
    FUNC.START
    RESERVE(1)
    mov edx, HASH.BONUS_WEAPON2 << 16
    mov [LOCAL(0)], edx
...
;update(dword *map)
;move all the bonus weapon2
global bonus_weapon2.update
bonus_weapon2.update:
    FUNC.START
    RESERVE(4)
...
; weapon2.put_all_in_map(dword *map)
weapon2.put_all_in_map:
    FUNC.START
    RESERVE(3) ; i, row, col
    cmp dword [count], 0
    ...
    .map.all.while:
        ...
        CALL weapon2.put_one_in_map, [PARAM(0)], edx, [LOCAL(1)], [LOCAL(2)]
...
; weapon2.put_one_in_map(dword *map, dword hash, dword row, dword col)
weapon2.put_one_in_map:
    FUNC.START
...
global bonus_weapon2.collision
bonus_weapon2.collision:
    FUNC.START    
...
;paint()
;move all the weapon2s
global bonus_weapon2.paint
bonus_weapon2.paint:
    FUNC.START
    RESERVE(4)
    ...
    set.form2:
        ...
        mov dword [graphics], 6|FG.YELLOW|BG.BLACK
        jmp while.internal

    set.form1:
        mov byte [graphics.style], 1
        mov dword [graphics], 6|FG.BLUE|BG.BLACK
        jmp while.internal

; bonus_weapon2.take_damage(dword damage, dword instance)
; Takes weapon2 away from an enemy
global bonus_weapon2.take_damage
bonus_weapon2.take_damage:
    FUNC.START
    RESERVE(1)
...
; bonus_weapon2.reset()
; reset the weapon2 enemies
global bonus_weapon2.reset
bonus_weapon2.reset:
    FUNC.START
    mov dword[count], 0
    FUNC.END